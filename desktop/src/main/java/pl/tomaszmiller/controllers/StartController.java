...
import pl.tomaszmiller.auth.KeycloakPasswordGrantClient;
...
public class StartController implements Initializable {
    ...
    private void login(ActionEvent event) {
        ...
        try {
            String issuer = System.getenv().getOrDefault("ALEXANDRIA_OIDC_ISSUER", "http://localhost:8081/realms/alexandria");
            String clientId = System.getenv().getOrDefault("ALEXANDRIA_OIDC_CLIENT_ID", "alexandria-swagger");
            String clientSecret = System.getenv().getOrDefault("ALEXANDRIA_OIDC_CLIENT_SECRET", "change-me");
            String token = new KeycloakPasswordGrantClient(issuer, clientId, clientSecret)
                    .loginAndGetAccessToken(emailValue, passwordValue);
            DesktopAuthSession.setAccessToken(token);
        } catch (Exception e) {
            LOGGER.warn("OIDC login failed.", e);
